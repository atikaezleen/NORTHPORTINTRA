...
<?php 
	if ( isset($_GET['id']) ) {
		$post_id = $_GET['id'];
		$post_title = "";
		$sql = "SELECT * FROM test.cms_post WHERE post_id = '$post_id'";
		$exec = pg_query($sql);
		if ($title = pg_fetch_assoc($exec)) {
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-8">
				...
				<?php
					if( isset($_GET['id'])) {
						$query = "SELECT * FROM test.cms_post WHERE post_id = '$_GET[id]'";
						$exec = pg_query($query);
						if (pg_num_rows($exec) > 0) {
							...
							<div class="post">
								...
								<div class="post-info">
									<p class="lead">
									Published on: <?php echo htmlentities($post_date); ?> | Department: <?php echo htmlentities($post_category);?> | By: <?php echo $post_author; ?>
									</p>
								</div>
				...
				<?php
					$sql = "SELECT * FROM test.comment WHERE post_id = '$_GET[id]'";
					$exec = pg_query($sql);
					if (pg_num_rows($exec) > 0) {
						while ($comments = pg_fetch_assoc($exec)) {
