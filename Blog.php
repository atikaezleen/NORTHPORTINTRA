...
<div class="blog">
	<nav class="navbar navbar-inverse" role="navigation">
		<div class="container">
			<div class="navbar-header">
				...
				<img src="Upload/Image/1.jpg" class ="navbar brand" style="background-color: #f9f9f9; margin-top: 2px;margin-bottom: 2px;"
                        height="95" width="">
				</a>
			</div>
	...
	<div class="container">
		<div class="blog-title">
			<div class="row">
				<div class="col-md-8 ">
					<h1 class="text-warning">Welcome To ISD Blog </h1>
					<p class="lead">By ELIN</p>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-md-8">
				<?php
					...
					if ( isset($_GET['search'])) {
						if ( empty($_GET['search'])) {
							...
							$search = $_GET['search'];
							$query = "SELECT * FROM test.cms_post WHERE post_date_time LIKE '$search' AND title LIKE '$search' AND category LIKE '$search' ";
						}
					}else if ( isset($_GET['category'])) {
						$query = "SELECT * FROM test.cms_post WHERE category = '$_GET[category]'";
						...
						$query = "SELECT * FROM test.cms_post ORDER BY post_date_time DESC LIMIT $showPost,5	";

					}else{

						$query = "SELECT * FROM test.cms_post ORDER BY post_date_time DESC OFFSET 0 LIMIT 5	";						
					}
					...
					if( $exec ) {
						if (pg_num_rows($exec) > 0) {
							...
							<div class="post">
								...
								<div class="post-info">
									<p class="lead">
									Published on: <?php echo htmlentities($post_date); ?> | Department: <?php echo htmlentities($post_category);?>
									</p>
								</div>
				...
				<?php
					...
					$sql = "SELECT COUNT(*) FROM test.cms_post";
					$exec = pg_query($sql);
					$rowCount = pg_fetch_array($exec);
			...
			<div class="col-md-3 col-md-offset-1 post-side-menu">
				...
				<div class="panel panel-primary">
					...
					<div class="panel-body">
						<?php
							$sql = "SELECT * FROM test.cms_post ORDER BY post_date_time LIMIT 5";
							$exec = pg_query($sql);
							while ($recentPost = pg_fetch_assoc($exec)) {
				...
				<div class="panel panel-primary">
					...
					<div class="panel-body">
						...
						<?php 
							...
							$exec = pg_query($sql);
							if (pg_num_rows($exec) > 0) {
								while ($category = pg_fetch_assoc($exec)) {
									?>
...
<div class="row navbar-inverse" id="blog-footer">
	<div class="footer-wrapper">
		<p>All Rights Reserved 2019| Theme By : ISD Department</p>
	</div>
</div>
</body>
</html>