...
if ( isset($_POST['submit'])) {
	...
	$confirmPassword = pg_escape_string($con,$_POST['confirm_password']);
	...
	if ( empty($username) || empty($password) || empty($confirmPassword)) {
		...
		$sql = "INSERT INTO test.cms_admin (date_time, username, password, added_by) VALUES('$dateTime', '$username', '$password', '$creator')";
		$exec = pg_query($sql);
		if ($exec) {
...
if ( isset($_GET['del_admin'])) {
	if ( !empty($_GET['del_admin'])) {
		$sql = "DELETE FROM test.cms_admin WHERE id = '$_GET[del_admin]'";
		$exec = pg_query($sql);
		if ($exec) {
				...
				<div>
					...
					<div id="cat_table">
						...
						<table class="table table-striped table-hover">
							...
							<?php
								$num = 1;
								$viewSql = "SELECT * FROM test.cms_admin ORDER BY date_time DESC";
								$exec = pg_query($viewSql);
