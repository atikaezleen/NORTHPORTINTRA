...
if ( isset( $_POST['post-update'])) {
	...
	$imageDirectory = "Upload/News/" . basename($_FILES['post-image']['name']);
	$sql = "UPDATE test.tbl_post SET news_date_time ='$dateTime', title = '$title', category = '$category', image = '$updatedImage', post = '$content' WHERE news_id = '$_POST[idFromUrl]' ";
	$exec = pg_query($sql);
	if($exec) {
	...
	if (!empty($_GET['news_id'])) {
		$sql = "SELECT * FROM test.tbl_post WHERE news_id = '$_GET[news_id]'";
		$exec = pg_query($sql);
